...
const INPUT_FILENAME: &str = "input/day_one_input.txt";
...
fn get_fuel_for_module(mass: Mass) -> Fuel {
  ...
  while current_step_mass != 0 {
    let current_step_fuel: Fuel = get_fuel_for_module_step(current_step_mass);
    total_fuel += current_step_fuel;
    current_step_mass = current_step_fuel as Mass;
  }
...
fn get_fuel_for_module_step(mass: Mass) -> Fuel {
  let one_third: Mass = (mass as f64 / 3.0).trunc() as Fuel;
  if one_third > 2 {
    one_third - 2
  } else {
    0
